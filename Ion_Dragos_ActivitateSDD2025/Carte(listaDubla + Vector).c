...
void inserareNodLS(nodLS** cap, nodLS** ultim, Carte c) {
	...
	nou->inf.vectPreturi = (float*)malloc(nou->inf.nrPreturi * sizeof(float));
...
void citireCartiDinVector(Carte* vect, int n, int limit) {
	for (int i = 0; i < n && i < limit; i++) {
		...
		for (int j = 0; j < vect[i].nrPreturi; j++)
			printf("Pret = %f", vect[i].vectPreturi[j]);
		printf("\n");
	}
}
...
int main() {
	...
	citireCartiDinVector(vect, nr, 3);
