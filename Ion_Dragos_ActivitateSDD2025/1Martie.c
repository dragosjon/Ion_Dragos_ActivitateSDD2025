...
struct Client citireClientFisier(FILE* f)
{
	...
	char buffer[100];
	char sep[3] = ",\n";
	fgets(buffer, 100, f); //citire fisier
	c.id = atoi(strtok(buffer, sep)); //atoi()=transforma sirul de caractere in intreg
	c.varsta = atoi(strtok(NULL, sep)); //primul parametru null pentru ca functia stie ca se pastreaza bufferul, daca pun din nou buffer o va lua de la 0 
	char* aux;
	aux = strtok(NULL, sep);
