...
void inserareNodLS(nodLS** cap, nodLS** ultim, Produs c) {
...
void salvareCartiInVector(nodLS* cap, Produs* vect, int* n) {
	nodLS* temp = cap;
	while (temp) {
		vect[*n] = temp->inf;
		(*n)++;
...
void citireCartiDinVector(Produs* vect, int n, int limit) {
